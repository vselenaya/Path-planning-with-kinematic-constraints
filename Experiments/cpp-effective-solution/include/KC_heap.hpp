...
вершина по этому индексу снова не используема и может быть выделена при новом запросе). В случае, если размер векторов
...
struct MyHEAP {
    ...
    */
    ...
    ptrVertex new_Vertex(int i, int j, int type, int info);
