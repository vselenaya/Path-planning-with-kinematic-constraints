...
static inline void set_parent(void *p, ptrSearchNode current, ptrSearchNode new_node) {
    /*
    Данная функция должна установить родителя для вершины поиска new_node, котоорая является
    ...
    перебором подобрать прмиитивы между этими целевыми ячейками, чтобы они стыковались друг с другом
    ...
    */
    ...
    if (current->vertex->type == -1) {  // если тип вершины = -1, то это дискретное состояние -> поиск на state lattice->
        new_node->parent = current;  // -> просто установили родтеля
        return;
    }
