...
void SearchTree::add_to_closed(ptrSearchNode item) {
    ...
    */
    ...
    //HEAP->delete_Vertex(item);  // больше нам вершина не нужна (так как попавшие в CLOSED больше не трогаются -> удаляем)  !!! так не делаем -удалим в деструкторе вместе с SearchNode
}
...
long double euclid_dist(int i1, int j1, int i2, int j2) {
    /*
    Функция вычисляет евклидово расстояние между проекциями вершин (i1,j1) и (i2,j2).
    */

    return sqrtf64x((long double) (i1 - i2) * (i1 - i2) + 
                    (long double) (j1 - j2) * (j1 - j2));
}
