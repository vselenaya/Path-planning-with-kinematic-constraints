    ...
    */
    ...
    if (mode == "COST" || mode == "PRIM") {
        ...
        if (res.find_path == 0)
            ...
            ptrSearchNode node = res.final_node;
            while (!(node == NULL_Node)) {  // так как мы специально хранили в качестве parent ольцо целевые вершины, то просто проходимся по нима
...
int main() {
    ...
    for (size_t i = 0; i < starts.size(); i ++) {
        Vertex *start = new Vertex(get<0>(starts[i]), get<1>(starts[i]), get<2>(starts[i]));  // генерируем начальное и винишное состояние 
    ...
    */
    ...
    vector <string> maps = {"Milan_1_256", "Moscow_0_512", "Labyrinth", "WheelofWar"};  // карты, на которых хотим тестировать
    vector <string> cs = {"big_control_set", "short_control_set"};  // control_set  "main_control_set"
    vector <string> types = {"big_types", "short_types"};  // соответствующие типы  "main_types"
